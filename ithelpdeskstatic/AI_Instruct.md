...
- admin.html     → Trang quản trị (upload ảnh, xuất/nhập JSON, bảo vệ mật khẩu)
- script.js      → Logic chung cho cả 2 trang
- styles.css     → Style toàn bộ
- data.js        → Dữ liệu ảnh (load từ file thay vì localStorage)
...
- [x] Accordion
- [x] Modal phòng họp
- [x] Admin upload ảnh (lưu base64 vào data.js)
- [x] Xuất/Nhập ảnh qua file data.json
- [x] Lightbox xem ảnh phóng to
- [x] Loading screen animation
- [x] Bảo vệ trang admin bằng mật khẩu (prompt khi load, sai → redirect về index.html)

Trạng thái hiện tại (cập nhật: 2026-05-11):
- Vừa xong: Thêm password protection cho admin.html (script inline, mật khẩu prompt khi vào trang)
- Vừa xong: Format lại HTML admin.html — căn chỉnh indentation, đóng thẻ <div class="desc-box"> đúng chỗ (section VPN & section App Mobile)
- Vừa xong: Cập nhật launch.json trỏ đúng thư mục WIP_11.5 (trước đó trỏ WIP_4.5)
- Trạng thái: Code đang hoạt động ổn định
...
*Tài liệu cập nhật vào 2026-05-11.*
